...
# ----------------- FRESH DATA STORAGE FILES (RESET) -----------------
DATA_FILE = "warehouse_entries.csv"
REPORT_FILE = "dispatch_reports.csv"

# Fresh Sheet Initialization: Delete old files if present to start completely fresh
...
# ----------------- MAIN LAYOUT: TWO COLUMNS (FORM & DASHBOARD) -----------------
...
with col2:
    ...
    if not df.empty:
        ...
        if not df_filtered.empty:
            ...
            st.info(f"No entries found for {selected_date_str}. Start adding entries from the left form.")
    else:
